...
class VectorStore(ABC):
    ...
    @abstractmethod
    def add_documents(self, documents: List[Document], collection: str, **kwargs: Any) -> List[str]:
        """Add documents to the store."""
        pass

    @abstractmethod
    async def aadd_documents(
        self, documents: List[Document], collection: str, **kwargs: Any
    ) -> List[str]:
    ...
    @abstractmethod
    def create_collection(self, name: str, dimension: int, **kwargs: Any) -> bool:
    ...
    def search_text(
        ...
        **kwargs: Any,
        ...
        return RetrievalResult(documents=[], query=query, total_results=0, search_time_ms=0.0)
...
class ChromaVectorStore(VectorStore):
    ...
    def search(
        ...
        **kwargs: Any,
        ...
        results = collection_obj.query(
            query_embeddings=[query_vector], n_results=k, where=filters, **kwargs
        )
    ...
    def search_text(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            results = collection_obj.query(
                query_texts=[query], n_results=k, where=filters, **kwargs
            )
            ...
            search_time_ms = (time.time() - start_time) * 1000
            return RetrievalResult(
                documents=[], query=query, total_results=0, search_time_ms=search_time_ms
            )
    ...
    def add_documents(self, documents: List[Document], collection: str, **kwargs: Any) -> List[str]:
        ...
        try:
            ...
            collection_obj = client.create_collection(
                name=collection, metadata=kwargs.get("metadata", {})
            )
    ...
    def create_collection(self, name: str, dimension: int, **kwargs: Any) -> bool:
        ...
        client.create_collection(
            name=name, metadata=kwargs.get("metadata", {"dimension": dimension})
        )
        return True
            ...
            try:
                ...
                for i, (doc, metadata) in enumerate(doc_metadata_pairs):
                    pinecone_vectors.append(
                        {"id": doc.id, "values": all_embeddings[i], "metadata": metadata}
                    )
...
def create_vector_store(store_type: str, **config: Any) -> VectorStore:
    ...
    instance: VectorStore = store_class(**config)
    return instance
